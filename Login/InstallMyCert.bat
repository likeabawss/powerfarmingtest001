...
set scriptDir=%~dp0
echo Running powershell.exe \\powerfarming.co.nz\NETLOGON\svn-netlogon\Login\InstallMyCert.ps1
REM Note the RemoteSigned in case the system is defaulted to restricted
powershell.exe -ExecutionPolicy RemoteSigned -command "& { "\\powerfarming.co.nz\NETLOGON\svn-netlogon\Login\InstallMyCert.ps1"; exit $LastExitCode }"
if "%ERRORLEVEL%" NEQ "0" (
...
powershell.exe -command \\powerfarming.co.nz\NETLOGON\svn-netlogon\Login\InstallMyCert.ps1
pause
