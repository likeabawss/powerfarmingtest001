...
' Date: 06/05/2011
	...
	'Server 2003 DC
	cmd = chr(34) & exepath & "LogParser.exe " & chr(34) & "-i:EVT " & chr(34) & "SELECT EventLog, RecordNumber, TO_UTCTIME(TimeGenerated), EventID, EventType, EventTypeName, EventCategory, SourceName, ComputerName, Message FROM System, Application, C:\WINDOWS\system32\config\NTDS.Evt, C:\WINDOWS\system32\config\DnsEvent.Evt, C:\WINDOWS\system32\config\NtFrs.Evt TO Events" & chr(34) &  " -o:SQL -server:logparserdb.powerfarming.co.nz -driver:" & chr(34) & "SQL Server" & chr(34) & " -database:" & GetComputerName() & " -username:logcollector -password:Dnalt0cs -createtable:ON -iCheckPoint:c:\support\logparse_evtcheck_F.lpc"
