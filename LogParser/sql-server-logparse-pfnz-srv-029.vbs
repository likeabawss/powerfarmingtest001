...
' Date: 06/05/2011
	...
	cmd = chr(34) & exepath & "LogParser.exe " & chr(34) & "-i:EVT " & chr(34) & "SELECT EventLog, RecordNumber, TO_UTCTIME(TimeGenerated), EventID, EventType, EventTypeName, EventCategory, SourceName, ComputerName, Message FROM System, Application, DFS\u0020Replication, Directory\u0020Service, DNS\u0020Server, File\u0020Replication\u0020Service  TO Events" & chr(34) &  " -o:SQL -server:logparserdb.powerfarming.co.nz -driver:" & chr(34) & "SQL Server" & chr(34) & " -database:PFWITMGMT -username:logparserdb -password:CakeCaution3 -createtable:ON -iCheckPoint:c:\support\logparse_evtcheck_1A.lpc"	
